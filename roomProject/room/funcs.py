...
def get_html(url, _keyword):
    ...
    #크롬 옵션 코드, 테스할때 씀
    html = list()
    driver = webdriver.Chrome("/Users/seni/Desktop/crowling test/chromedriver")
...
def parse_html(html):
    ...
    for html_idx in html:
        ...
        room_area = soup.find_all("div",{"class": "hotel-info"})
        ...
        for room_idx in room_area:
            ...
            with urlopen(imgUrl) as f:
                with open('./img/' + _title +'.jpg','wb') as h:
                    img = f.read() #이미지 읽기
                    h.write(img)
            '''
...
def set_weight(_input):
    weight_li = [0, 0, 0, 0, 0]
    dic = {0 :_input[0], 1 :_input[1], 2 :_input[2], 3 :_input[3], 4 :input[4]}
    dic.sort()
    weight = 0.1
    for idx in dic.keys:
